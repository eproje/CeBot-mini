#include "CBMRGBLed.h"
/*
CBMRGBLed::CBMRGBLed(void) 
{
  pinMode(DEFAULT_LED_PIN, OUTPUT);
  _pin = DEFAULT_LED_PIN;
  pinMask       = digitalPinToBitMask(_pin);
  ws2812_port   = portOutputRegister(digitalPinToPort(_pin) );
  setNumber(DEFAULT_MAX_LED_NUMBER);
}
*/  

CBMRGBLed::CBMRGBLed(int pin) 
{
  pinMode(pin, OUTPUT);
  _pin = pin;
...
void CBMRGBLed::setNumber(uint8_t num_leds)
{
...
bool CBMRGBLed::setColor(uint8_t index, uint8_t red, uint8_t green, uint8_t blue)
{
...
bool CBMRGBLed::setColorAt(uint8_t index, uint8_t red, uint8_t green, uint8_t blue)
{
  if(index < count_led)
  {
    uint8_t tmp = index * 3;
...
void CBMRGBLed::rgbled_sendarray_mask(uint8_t *data, uint16_t datlen, uint8_t maskhi, uint8_t *port)
{
